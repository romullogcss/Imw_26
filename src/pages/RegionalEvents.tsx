...
import { EVENT_HEADER_CONFIGS } from '../data/churchData';
...
interface RegionalEventsPageProps {
...
export const RegionalEventsPage: React.FC<RegionalEventsPageProps> = ({ onNavigate }) => {
  ...
  const regionalEvents = eventsList.filter((e) => e.eventType === 'regional');
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 pb-20">
      ...
      <EventsPageHeader
        title={EVENT_HEADER_CONFIGS.regional.title}
        description={EVENT_HEADER_CONFIGS.regional.description}
        backgroundImageUrl={EVENT_HEADER_CONFIGS.regional.backgroundImageUrl}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-10">
        <div className="flex items-center justify-between gap-4 mb-6 border-b border-slate-200 pb-4">
          <div>
            <span className="text-purple-700 font-extrabold text-xs uppercase tracking-widest block">
              REGIÃO ECLESIÁSTICA
            </span>
            <h2 className="text-2xl font-black text-slate-900 uppercase">
              Agenda Regional ({regionalEvents.length})
            </h2>
          </div>
        </div>
        ...
        {regionalEvents.length === 0 ? (
          <div className="bg-white rounded-2xl p-12 text-center border border-slate-200 shadow-sm space-y-4 max-w-xl mx-auto">
            ...
            <h3 className="text-lg font-black uppercase text-slate-800">
              Nenhum Evento Regional Programado
            </h3>
            <p className="text-slate-600 text-xs leading-relaxed">
              No momento não há congressos ou eventos regionais cadastrados. Você pode consultar a agenda de nossa igreja local ou os eventos distritais.
            </p>
                ...
                <div>
                  ...
                  <div className="relative h-48 bg-slate-100 overflow-hidden">
                    ...
                    <div className="absolute top-3 left-3 flex items-center gap-2">
                      <span className="px-2.5 py-1 rounded-md bg-purple-700 text-white font-black text-[10px] uppercase shadow-sm">
                        🏛️ REGIONAL
                      </span>
                      {event.badge && (
                  ...
                  <div className="p-5 space-y-3">
                    ...
                    <div className="space-y-1.5 text-xs text-slate-600">
                      <p className="font-bold text-purple-800 flex items-center gap-1.5">
                        <Calendar className="w-4 h-4 shrink-0 text-purple-700" />
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 space-y-4 shadow-2xl relative border border-slate-200">
            ...
            <div className="border-b border-slate-100 pb-3">
              <span className="text-purple-700 text-xs font-black uppercase tracking-wider block">
                Inscrição para Evento Regional
              </span>
            ...
            {signupSuccess ? (
              <div className="py-8 text-center space-y-3">
                ...
                <p className="text-xs text-slate-600">
                  Sua inscrição no evento regional foi realizada com sucesso.
                </p>
                <button
                  onClick={() => setSelectedEventForSignup(null)}
